...
(p_emp_id NUMBER,
p_emp_increment NUMBER)
IS
-- 선언부
    v_sal employees.salary%TYPE;
    v_result_sal v_sal%TYPE;
BEGIN
    -- 사번에 따른 급여 조회
    SELECT salary
    INTO v_sal
    ...
    v_result_sal := v_sal + v_sal * (p_emp_increment / 100);
    
    -- 새로 연산된 급여 갱신
    UPDATE employees
    SET salary = v_result_sal
    WHERE employee_id = p_emp_id;

EXCEPTION
    WHEN NO_DATA_FOUND THEN -- 입력한 사원이 없는 경우는 SELECT문 결과가 없다는 뜻->NO_DATA_FOUND 에러이용
        DBMS_OUTPUT.PUT_LINE('No search employee!!');
END;
/

EXECUTE y_update(200, 10);
EXECUTE y_update(2000, 50);
